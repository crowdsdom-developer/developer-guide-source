...
AnswerSpec | 本问题答案的相关设定，根据答案类型有所不同，在下面的段落有详细的描述。 | Object | None | Yes
...
为了让请求者可以辨别答案来源的工作者及指定工作，系统在载入 iframe 的 url 时，会在请求者所提供的 url 后面加入数个参数： taskId, jobId, submitTo, contributorId。这些参数将以query的方式传入：`?`之后为附上的参数, `=` 前面为参数的名称、后面为参数的值，参数之间用`&`隔开。

例如，本来的 url 为：

` https://awesometask.crowdsdom.com/fantasticsurvey?surveyId=101`
...
`&submitTo=https://www.crowdsdom.com/tasks/externalSubmit`
`&conributorId=A256GJL6JKFB`
...
外部问题表单必须提交结果至 `https://www.crowdsdom.com/tasks/externalSubmit` 来完成指定工作。 在表单提交时必须包含 taskId，这样我们的系统才有办法判断该答案所对应的指定工作。
...
```javascript
questionsForm: {
  url: 'https://awesometask.crowdsdom.com/fantasticsurvey?surveyId=101',
  frameHeight: 400
}
```
